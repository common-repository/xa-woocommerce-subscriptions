...
class Hforce_Date_Time_Utils {
    ...
    public static function subscription_period_strings($number = 1, $period = '') {
        ...
        $t_day = __('day', HForce_Woocommerce_Subscription_Admin::TEXT_DOMAIN);
        $t_week = __('week', HForce_Woocommerce_Subscription_Admin::TEXT_DOMAIN);
        $t_month = __('month', HForce_Woocommerce_Subscription_Admin::TEXT_DOMAIN);
        $t_year = __('year', HForce_Woocommerce_Subscription_Admin::TEXT_DOMAIN);
        
        $t_days = __('days', HForce_Woocommerce_Subscription_Admin::TEXT_DOMAIN);
        $t_weeks = __('weeks', HForce_Woocommerce_Subscription_Admin::TEXT_DOMAIN);
        $t_months = __('months', HForce_Woocommerce_Subscription_Admin::TEXT_DOMAIN);
        $t_years = __('years', HForce_Woocommerce_Subscription_Admin::TEXT_DOMAIN);
        
        $subscription_periods = apply_filters('hf_subscription_periods', array(
            'day' => sprintf(_nx($t_day, '%s '.$t_days, $number, 'Subscription billing period.', HForce_Woocommerce_Subscription_Admin::TEXT_DOMAIN), $number),
            'week' => sprintf(_nx($t_week, '%s '.$t_weeks, $number, 'Subscription billing period.', HForce_Woocommerce_Subscription_Admin::TEXT_DOMAIN), $number),
            'month' => sprintf(_nx($t_month, '%s '.$t_months, $number, 'Subscription billing period.', HForce_Woocommerce_Subscription_Admin::TEXT_DOMAIN), $number),
            'year' => sprintf(_nx($t_year, '%s '.$t_years, $number, 'Subscription billing period.', HForce_Woocommerce_Subscription_Admin::TEXT_DOMAIN), $number),
                )
        );
...
public static function is_mysql_datetime_format( $time ) {
    ...
    public static function hforce_get_subscription_ranges($subscription_period = '') {
        ...
        $locale = get_locale();
        $subscription_ranges = HForce_Woocommerce_Subscription_Admin::get_subscription_ranges('hf-subs-ranges-' . $locale, 'get_original_subscription_ranges', array(), 3 * HOUR_IN_SECONDS);        
