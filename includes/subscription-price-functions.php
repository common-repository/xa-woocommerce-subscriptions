...
function hforce_price_string($subscription_details) {
    ...
    $subscription_period_string = __($subscription_period_string, HForce_Woocommerce_Subscription_Admin::TEXT_DOMAIN);
    
    $subscription_ranges = Hforce_Date_Time_Utils::hforce_get_subscription_ranges();
    $t_every = __('every', 'xa-woocommerce-subscription');
    ...
    if ($subscription_details['subscription_length'] > 0 && $subscription_details['subscription_length'] == $subscription_details['subscription_interval']) {
        ...
        $subscription_string = sprintf(_n('%1$s %2$s then %3$s / %4$s', '%1$s %2$s then %3$s '.$t_every.' %4$s', $subscription_details['subscription_interval'], 'xa-woocommerce-subscription'), $initial_amount_string, $subscription_details['initial_description'], $recurring_amount_string, $subscription_period_string);
    } elseif (!empty($subscription_details['recurring_amount']) || intval($subscription_details['recurring_amount']) === 0) {
        $subscription_string = sprintf(_n('%1$s / %2$s', '%1$s '.$t_every.' %2$s', $subscription_details['subscription_interval'], 'xa-woocommerce-subscription'), $recurring_amount_string, $subscription_period_string);
    } else {
        $subscription_string = '';
    }
